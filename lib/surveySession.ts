...
export async function ensureUserAndSession(
    ...
    // ต้องมี user ของ Supabase Auth ก่อน (ล็อกอินแล้ว)
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error("not authenticated")

    let lid = (lineUserId ?? "").trim()
    if (!lid && typeof window !== "undefined") {
        lid = (sessionStorage.getItem("line_user_id") ?? "").trim()
    }
    if (!lid) throw new Error("missing lineUserId")

    if (typeof window !== "undefined") {
        sessionStorage.setItem("line_user_id", lid)
    }

    // ⬇️ upsert โปรไฟล์ลงตาราง profiles (ไม่ใช่ users)
    await supabase
        .from("profiles")
        .upsert(
            {
